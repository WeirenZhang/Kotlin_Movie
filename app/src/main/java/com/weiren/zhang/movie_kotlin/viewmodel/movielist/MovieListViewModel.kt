...
constructor(private val mDailyApi: MovieListApi) : BaseViewModel() {
    ...
    fun getMovieList(isRefreshing: Boolean, Home_ID: String): LiveData<List<MovieListModel>> =
        liveDataEx {
            if (isRefreshing) {
                Page = 1
            }
            val movieListModels = mutableListOf<MovieListModel>()
            val dailyModel = mDailyApi.getMovieList(Page.toString(), "MovieList", Home_ID)
            ...
            /*
            val movieListModels = mutableListOf<MovieListModel>()
            val dailyModel = mDailyApi.getMovieComingsoonList(Page.toString())
