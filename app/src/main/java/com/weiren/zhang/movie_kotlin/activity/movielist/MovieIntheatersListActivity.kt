...
import com.weiren.zhang.movie_kotlin.viewmodel.movielist.MovieIntheatersListViewModel

@AndroidEntryPoint
@Route(path = RouterPath.MovieList.PATH_InTheaters_HOME)
class MovieIntheatersListActivity :
    BaseBindVMActivity<MovieIntheatersListViewModel, RecyclerviewBinding>() {
    ...
    private fun getMovieList() {
        mViewModel.getMovieList(false).observerKt {
            println("MovieIntheatersListActivity size:" + it.size)
            if (it.isNotEmpty()) {
                println("isNotEmpty")
