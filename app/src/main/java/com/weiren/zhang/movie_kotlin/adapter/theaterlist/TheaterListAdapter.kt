...
import com.weiren.zhang.movie_kotlin.databinding.TheaterListItemBinding
import com.weiren.zhang.movie_kotlin.model.theaterlist.TheaterInfoModel

class TheaterListAdapter(private val mActivity: Activity) :
    BaseQuickAdapter<TheaterInfoModel, BaseViewHolder>(R.layout.theater_list_item), LoadMoreModule {
    ...
    init {
        setOnItemClickListener { adapter, view, position ->
            ...
            ARouter.getInstance().build(RouterPath.Theater.PATH_TheaterResult_HOME)
                .withString(BaseConstant.Theater_ID_KEY, toJson(itemData))
    ...
    override fun convert(holder: BaseViewHolder, item: TheaterInfoModel) {
        val binding = DataBindingUtil.getBinding<TheaterListItemBinding>(holder.itemView)
        binding?.model = item
    }
}